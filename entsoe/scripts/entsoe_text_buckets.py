...
def assign_bucket(text):
    ...
    if any(
        w in t
        for w in ["yearly maintenance", "annual maintenance", "foreseen maintenance"]
    ):
    ...
    if "start" in t and (
        "fault" in t or "problem" in t or "not ready" in t or "block" in t
    ):
...
LLM_BUCKETS_PATH = os.path.join(_ROOT_DIR, "reason_text_buckets.json")
...
for zone in prices.zone.unique():
    ...
    print(
        f"  {zone}: min={profile.min():.1f} (h={profile.idxmin()}), max={profile.max():.1f} (h={profile.idxmax()})"
    )
...
ax.set_title(
    "Price Impact by Text Bucket × Time Horizon\n(single-point, all zones 2016–2025)",
    fontsize=14,
    fontweight="bold",
)
...
ax.set_title(
    "Hourly Price Trajectory by Text Bucket\n(top 12 buckets, all zones 2016–2025)",
    fontsize=14,
    fontweight="bold",
)
...
ax.set_title(
    "Price Change Distribution at +6h by Text Bucket\n(top 15 buckets, all zones 2016–2025)",
    fontsize=14,
    fontweight="bold",
)
...
print("=" * 80)
print(
    f"{'Bucket':<35s} {'n':>6s} {'%unpl':>6s} {'+1h':>7s} {'+3h':>7s} {'+6h':>7s} {'+12h':>7s} {'+24h':>7s}"
)
print("-" * 80)
...
for _, row in mdf.iterrows():
    ...
    print(
        f"{row.bucket:<35s} {row.n:>6.0f} {pct_unplanned:>5.0f}% "
...
for zone in zones:
    ...
    plt.colorbar(im, ax=ax, label="Avg price change (EUR/MWh)", shrink=0.8)
    ax.set_title(
        f"{zone} — Price Impact by Text Bucket × Time Horizon\n(2016–2025)",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_xlabel("Hours after outage start")
    plt.tight_layout()
    plt.savefig(
        f"{FIG_DIR}/entsoe_bucket_heatmap_{zone}.png", dpi=150, bbox_inches="tight"
    )
...
for zone in zones:
    ...
    for bucket in zone_good:
        ...
        for h in HORIZONS_FINE:
            row[f"+{h}h"] = b_data[f"change_{h}h"].mean()
        vals = [
            abs(row[f"+{h}h"]) for h in HORIZONS_FINE if not np.isnan(row[f"+{h}h"])
        ]
        row["max_abs"] = max(vals) if vals else 0
    ...
    ax.set_xticklabels([f"{h}" for h in HORIZONS_FINE], fontsize=8)
    ax.set_title(
        f"{zone} — Hourly Price Trajectory by Text Bucket\n(top 10 buckets, 2016–2025)",
        fontsize=14,
        fontweight="bold",
    )
