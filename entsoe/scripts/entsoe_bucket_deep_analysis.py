...
BUCKETS_PATH = os.path.join(_ROOT_DIR, "reason_text_buckets.json")
...
for zone in umm.zone.unique():
    zone_prices = (
        prices[prices.zone == zone]
        .set_index("timestamp_utc")["price_eur_mwh"]
        .sort_index()
    )
...
bucket_order = (
    filtered.groupby("bucket")["change_6h"].mean().sort_values().index.tolist()
)
bucket_order = [b for b in bucket_order if b in good_buckets]

fig, axes = plt.subplots(
    1, len(mw_bins), figsize=(24, max(8, len(bucket_order) * 0.4)), sharey=True
)
...
ax.set_title(
    "Price Impact at +6h: Bucket × Zone\n(deseasonalized, 2016–2025)",
    fontsize=14,
    fontweight="bold",
)
plt.tight_layout()
...
ax.set_title(
    "Peak Price Impact & Recovery Time by Bucket\n(deseasonalized, all zones 2016–2025)",
    fontsize=14,
    fontweight="bold",
)
plt.tight_layout()
...
for zone in umm_active.zone.unique():
    ...
    for i in range(len(zone_evts)):
        ...
        active = np.sum((starts[:i] <= t) & (ends[:i] > t)) + np.sum(
            (starts[i + 1 :] <= t) & (ends[i + 1 :] > t)
        )
...
ax.set_title(
    "Cascade Effect: Price Impact by Number of Concurrent Outages\n(all zones 2016–2025)",
    fontsize=14,
    fontweight="bold",
)
...
for (lo, hi, label), color in zip(mw_cascade_bins, mw_colors):
    sub = impacts_mw[
        (impacts_mw.total_mw_offline >= lo) & (impacts_mw.total_mw_offline < hi)
    ]
    if len(sub) < 20:
        continue
    means = [sub[f"change_{h}h"].mean() for h in HORIZONS]
...
ax.set_xticklabels([str(h) for h in HORIZONS], fontsize=8)
ax.set_title(
    "Price Impact by Total MW Offline in Zone\n(all zones 2016–2025, deseasonalized)",
    fontsize=14,
    fontweight="bold",
)
ax.legend(fontsize=11)
...
for ax_i, zone in enumerate(cascade_zones):
    ...
    for (lo, hi, label), color in zip(zone_mw_bins, mw_colors):
        sub = zone_data[
            (zone_data.total_mw_offline >= lo) & (zone_data.total_mw_offline < hi)
        ]
        if len(sub) < 10:
            continue
        means = [sub[f"change_{h}h"].mean() for h in HORIZONS]
...
fig.suptitle(
    "Price Impact by Total MW Offline — Per Zone\n(deseasonalized, 2016–2025)",
    fontsize=15,
    fontweight="bold",
)
plt.tight_layout()
plt.savefig(
    f"{FIG_DIR}/entsoe_cascade_total_mw_per_zone.png", dpi=150, bbox_inches="tight"
)
...
print(
    f"  Better predictor: {'Total MW' if abs(r_mw) > abs(r_count) else 'Event count'}"
)
...
ax.set_yticklabels(
    [f"{b} (n={bucket_counts[b]:,})" for b in monthly_pct.index], fontsize=9
)
...
for _, row in adf.iterrows():
    color = (
        "#E74C3C"
        if row.significant and row.mean_6h > 0
        else "#3498DB"
        if row.significant and row.mean_6h <= 0
        else "#95A5A6"
    )
...
ax.legend(
    handles=[
        plt.Line2D(
            ...
            markersize=10,
            label="Significant positive (p<0.05)",
        ),
        plt.Line2D(
            [0],
            [0],
...
ax.set_title(
    "Training Signal Audit: Which Buckets Carry Predictive Value?\n"
...
print(f"{'=' * 110}")
print(
    f"{'Bucket':<30s} {'n':>5s} {'%Unpl':>6s} {'AvgMW':>7s} "
...
for _, row in adf.iterrows():
    ...
    print(
        f"{row.bucket:<30s} {row.n:>5.0f} {row.pct_unplanned:>5.0f}% {row.avg_mw:>6.0f} "
...
print(f"{'=' * 70}")
print(
    f"Buckets with significant POSITIVE signal (include in text): {len(sig_positive)}"
)
...
for _, row in not_sig.iterrows():
    print(
        f"  ~ {row.bucket}: {row.mean_6h:+.2f} EUR/MWh (p={row.p_value:.2f}, n={row.n:.0f})"
    )
