...
def compute_elo_ratings_multielo(rankings: list, models: list, base_rating: int = 1500,
                                  n_seeds: int = 32, k_value: int = 32) -> dict:
    """
...
def generate_context_mean_table(config: dict, output_path: str = None) -> str:
    ...
    """
    models = config.get('models', ['ttfm', 'chronos_univar', 'chronos_emb', 'timesfm_univar', 'gpt_forecast'])
    context_groups, has_horizon = flatten_config(config)
    precision = config.get('precision', 3)
    caption = config.get('caption_context_mean', 'Evaluation results averaged over horizons for each context and dataset.')
    ...
    for (context_len, dataset_name), rows in grouped_data.items():
        ...
        n_eval_samples_vals = [row['n_eval_samples'] for row in rows if 'n_eval_samples' in row and pd.notna(row['n_eval_samples'])]
        if n_eval_samples_vals:
            avg_row['n_eval_samples'] = int(n_eval_samples_vals[0])  # Take first value as they should all be the same
        else:
        ...
        for model in models:
            ...
            mean_vals = [row[mean_col] for row in rows if mean_col in row and pd.notna(row[mean_col])]
            median_vals = [row[median_col] for row in rows if median_col in row and pd.notna(row[median_col])]
    ...
    lines.append(f"    {empty_cols}& \\multicolumn{{{n_models}}}{{c}}{{Mean MAE ($\\downarrow$)}}")
    lines.append(f"      & \\multicolumn{{{n_models}}}{{c}}{{Mean MSE ($\\downarrow$)}} \\\\")
    lines.append(f"    \\cmidrule(lr){{{first_cols+1}-{first_cols+n_models}}} \\cmidrule(lr){{{first_cols+n_models+1}-{first_cols+2*n_models}}}")
    lines.append("    ")
    ...
    for idx, avg_row in enumerate(averaged_rows):
        ...
        display_name = all_dataset_names.get(dataset_name, 
                      DATASET_DISPLAY_NAMES.get(dataset_name, dataset_name)).replace(" (TimeMMD)", "$^*$").replace(" (FNSPID)", "$^{\\#}$")
        ...
        # Build row
        n_samples_str = str(int(avg_row['n_eval_samples'])) if 'n_eval_samples' in avg_row and pd.notna(avg_row['n_eval_samples']) else "-"
        if prev_context != context_len:
            lines.append(f"    \\textbf{{{context_len}}} & {display_name} & {n_samples_str}")
...
def generate_latex_table(config: dict, output_path: str = None) -> str:
    ...
    context_groups, has_horizon = flatten_config(config)
    precision = config.get('precision', 3)
    caption = config.get('caption', 'Evaluation results comparing TTFM against baseline models.')
    ...
    for (horizon, context_len), ctx_group_list in merged_groups.items():
        ...
        for row in all_rows:
            ...
            n_samples_str = str(int(row['n_eval_samples'])) if 'n_eval_samples' in row and pd.notna(row['n_eval_samples']) else "-"
            if has_horizon:
                if first_row:
                    h_col = f"\\textbf{{{horizon}}}" if show_horizon else ""
                    lines.append(f"    {h_col} & \\textbf{{{context_len}}} & {display_name} & {n_samples_str}")
                    first_row = False
                    show_horizon = False
                else:
                    lines.append(f"    & & {display_name} & {n_samples_str}")
            ...
            lines.append(f"    & {' & '.join(mean_vals)} & {' & '.join(median_vals)} \\\\")
            lines.append("    ")
...
def collect_improvement_data(config: dict, metric: str = 'median_mae', 
                             exclude_datasets: set = None,
                             ttfm_model: str = 'ttfm',
                             baseline_model: str = 'chronos_univar') -> dict:
    """
...
def collect_dual_improvement_data(config: dict, metric: str = 'median_mae',
                                   exclude_datasets: set = None) -> tuple:
    """
    ...
    chronos_data, all_names1 = collect_improvement_data(
        ...
        ttfm_model='ttfm', baseline_model='chronos_univar'
    )
    
    # Collect TTFM-TimesFM vs TimesFM improvement
    timesfm_data, all_names2 = collect_improvement_data(
        config, metric, exclude_datasets,
        ttfm_model='ttfm_timesfm', baseline_model='timesfm_univar'
    )
...
def collect_metric_values(config: dict, metric: str = 'median_mae',
                          exclude_datasets: set = None) -> dict:
    """
    ...
    for ctx_group in context_groups:
        ...
        for idx, row in df.iterrows():
            ...
            if pd.notna(chronos_val) and chronos_val < metric_data[dataset_name]['chronos']:
                metric_data[dataset_name]['chronos'] = chronos_val
    
    # Filter out datasets with inf values
    result = {k: v for k, v in metric_data.items() 
              if v['ttfm'] != float('inf') and v['chronos'] != float('inf')}
    return result, all_dataset_names


def plot_radar_comparison(config: dict, output_dir: Path, metric: str = 'median_mae',
                          exclude_datasets: set = None):
    """
    Generate ICML-style radar plot comparing TTFM vs Chronos2 across datasets.
    """
    metric_data, all_dataset_names = collect_metric_values(config, metric, exclude_datasets)
    ...
    display_names = [clean_latex_for_plot(
    ...
    chronos_norm = [0.25 + 0.4 * (1 - (v - min_val) / val_range) for v in chronos_values]
    ...
    # Plot data
    ax.plot(angles, ttfm_norm, 'o-', linewidth=2.5, label='TTFM (Ours)', 
            color=ttfm_color, markersize=8)
    ax.fill(angles, ttfm_norm, alpha=0.1, color=ttfm_color)
    
    ax.plot(angles, chronos_norm, 's-', linewidth=2.5, label='Chronos2', 
            color=chronos_color, markersize=8)
    ...
    for angle, label in zip(angles[:-1], display_names):
        ...
        if 90 < angle_deg < 270:
            ...
            rotation = angle_deg
            ha = 'right'
        ax.text(angle, 0.98, label_multiline, size=14, fontweight='medium',
...
def plot_improvement_by_context(config: dict, output_dir: Path, metric: str = 'median_mae',
                                exclude_datasets: set = None):
    """
    Generate combined bar plot showing max TTFM improvement over Chronos for each dataset.
    """
    improvement_data, all_dataset_names = collect_improvement_data(config, metric, exclude_datasets)
...
def plot_combined_improvement(improvement_data: dict, all_dataset_names: dict, 
                              output_dir: Path, metric: str = 'median_mae'):
    ...
    sorted_data = sorted(zip(datasets, max_improvements), 
    ...
    fig_width_inches = 10
    available_width_per_tick = (fig_width_inches * 0.8) / len(datasets) if datasets else 1.0
    ...
    bars = ax.bar(range(len(datasets)), max_improvements, 
                 color='#2ca02c', edgecolor='black', linewidth=0.8, width=0.7)
    
    # Add value labels with padding above bars
    for bar, imp in zip(bars, max_improvements):
        height = bar.get_height()
        padding = max(max(max_improvements) * 0.03, (max(max_improvements) - min(max_improvements)) * 0.005) if max_improvements else 0.5
    ...
    ax.set_title(f'Multimodal Gains in MAE', fontweight='normal', fontsize=20)
    ...
    max_lines = max(len(label.split('\n')) for label in wrapped_datasets) if wrapped_datasets else 1
    ...
    ax.set_ylim(bottom=min(0, y_min) if y_min < 0 else 0,
               top=y_max * 1.15 if y_max > 0 else 1)
...
def plot_elo_ratings(config: dict, output_dir: Path, metric: str = 'mean_mae'):
    ...
    sorted_data = sorted(zip(display_names, ratings, models), 
                        key=lambda x: x[1], reverse=True)
    ...
    bars = ax.bar(range(len(models)), ratings, 
    ...
    ax.axhline(y=1500, color='red', linestyle='--', linewidth=1.5, 
               label='Base Rating (1500)', alpha=0.7)
    
    metric_display = metric.replace('_', ' ').upper()
    ax.set_ylabel('ELO Rating', fontweight='bold', fontsize=12)
    ax.set_xlabel('Model', fontweight='bold', fontsize=12)
    ax.set_title(f'Model ELO Ratings ({metric_display})', fontweight='bold', fontsize=14)
...
def plot_combined_elo_and_improvement(config: dict, output_dir: Path, metric: str = 'mean_mae',
                                       exclude_datasets: set = None):
    ...
    chronos_improvement, timesfm_improvement, all_dataset_names = collect_dual_improvement_data(
        config, metric, exclude_datasets
    )
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 14), dpi=300, 
    ...
    for model in models:
        line1 = MODEL_COLUMNS[model]['line1']
        line2 = MODEL_COLUMNS[model].get('line2', '').replace('Forecast', '').replace('MV', '(MV)')
        if line2:
    ...
    # Sort by ELO rating (descending)
    sorted_elo = sorted(zip(display_names_elo, ratings, models), 
                        key=lambda x: x[1], reverse=True)
    display_names_elo, ratings, models_sorted = zip(*sorted_elo)
    
    # Color bars - highlight TTFM variants in green, others in muted red
    colors_elo = ['#4a8c4a' if m in ['ttfm', 'ttfm_timesfm'] else '#c45c5c' for m in models_sorted]
    
    bars1 = ax1.bar(range(len(models_sorted)), ratings, 
                    color=colors_elo, edgecolor='#333333', linewidth=0.8, width=0.6)
    ...
    ax1.set_xticklabels(display_names_elo, rotation=0, ha='center', fontsize=16, fontweight='bold')
    ...
    sorted_data = sorted(zip(display_names, chronos_vals, timesfm_vals, avg_improvements, dataset_names_list), 
    ...
    print(f"\n=== Max Improvements ===")
    print(f"TTFM vs Chronos2: {max(chronos_vals):.2f}% ({display_names[max_chronos_idx]})")
    print(f"TTFM-TimesFM vs TimesFM: {max(timesfm_vals):.2f}% ({display_names[max_timesfm_idx]})")
    ...
    bars_chronos = ax2.bar(x - bar_width/2, chronos_vals, bar_width, 
                           label='Chronos2', color=chronos_color, 
                           edgecolor='#333333', linewidth=0.8)
    bars_timesfm = ax2.bar(x + bar_width/2, timesfm_vals, bar_width,
                           label='TimesFM2.5', color=timesfm_color,
                           edgecolor='#333333', linewidth=0.8)
    ...
    ax2.set_xticks(x)
    ax2.set_xticklabels(display_names, rotation=45, ha='right', fontsize=18, fontweight='bold')
...
def generate_dataset_summary(config: dict, output_path: str = None, data_dir: str = None) -> str:
    """
    ...
    # Generate LaTeX table
    caption = config.get('summary_caption', 
    ...
    lines.append("    \\toprule")
    lines.append("    \\textbf{Dataset} & \\textbf{Target Source} & \\textbf{Text Source} & \\textbf{Frequency} & \\textbf{Date Range} & \\textbf{Samples} & \\textbf{$\\geq$2024-06} \\\\")
    lines.append("    \\midrule")
    ...
    for stats in dataset_stats:
        ...
        lines.append(f"    {display_name} & {target_source} & {text_source} & {frequency} & {date_range} & {total:,} & {post_2024:,} \\\\")
    
    lines.append("    \\midrule")
    lines.append(f"    \\textbf{{Total}} & & & & & \\textbf{{{total_all:,}}} & \\textbf{{{total_post_2024:,}}} \\\\")
    lines.append("    \\bottomrule")
    lines.append("    \\end{tabular}")
    lines.append("    ")
    lines.append(f"    \\caption{{{caption}}}")
...
def generate_dataset_summary_grouped(config: dict, output_path: str = None, data_dir: str = None) -> str:
    """
    ...
    lines.append("    \\toprule")
    lines.append("    \\textbf{Text Source} & \\textbf{Dataset} & \\textbf{Target Source} & \\textbf{Frequency} & \\textbf{Date Range} & \\textbf{Total} & \\textbf{$\\geq$2024-06} \\\\")
    lines.append("    \\midrule")
    ...
    for text_source in group_order:
        ...
        for stats in stats_list:
            ...
            if first:
                lines.append(f"    \\textbf{{{text_source}}} & {display_name} & {target_source} & {frequency} & {date_range} & {total:,} & {post_2024:,} \\\\")
                first = False
            else:
                lines.append(f"    & {display_name} & {target_source} & {frequency} & {date_range} & {total:,} & {post_2024:,} \\\\")
...
def create_standalone_latex(table_content: str, output_path: str):
    ...
    standalone_content = table_content.replace('\\begin{table*}[t]', '\\begin{table}[h]')
...
def main():
    ...
    parser.add_argument('--output', type=str, default='results_table.tex', help='Output LaTeX file path')
    parser.add_argument('--standalone', action='store_true', help='Also generate standalone LaTeX for PDF')
    parser.add_argument('--plot', action='store_true', help='Generate improvement plots')
    parser.add_argument('--plot_dir', type=str, default=None, help='Directory for plots (default: same as output)')
    parser.add_argument('--metric', type=str, default='median_mae',
    ...
    parser.add_argument('--summary', action='store_true', help='Generate dataset summary table')
    parser.add_argument('--summary-grouped', action='store_true', help='Generate dataset summary table grouped by text source')
    ...
    if args.config:
        ...
        if models_cfg is None or models_cfg == 'auto':
            ...
            if first_csv and os.path.exists(first_csv):
                ...
                config['models'] = config.get('models') or ['ttfm', 'chronos_univar', 'timesfm_univar']
    else:
        # Default config for demonstration
        config = {
            'models': ['ttfm', 'chronos_univar', 'chronos_emb', 'timesfm_univar', 'gpt_forecast'],
            'precision': 3,
    ...
    if args.plot:
        ...
        plot_combined_elo_and_improvement(config, plot_dir, args.metric, exclude_datasets)
    ...
    if getattr(args, 'summary_grouped', False):
        ...
        data_dir = args.data_dir
        grouped_content = generate_dataset_summary_grouped(config, grouped_output, data_dir)
