#!/usr/bin/env python3
"""
Merge Media Cloud news into TE news for electricity stems, filling only gap dates.

For each electricity stem (from electricity_queries.json): load TE news from
te_news_by_symbol/<stem>.json, raw/<stem>.csv (numeric date range). Gap dates = dates
in raw that have no TE text. Add MC entries only for gap dates, merge with TE
(TE first; MC fills gaps), sort by date. Write to data/te_electricity/merged_news/
so run_daily_summaries_by_symbol and merge_all_te_text can use the filled series.

Usage:
  uv run python scripts/te_electricity/merge_mc_into_te_gaps.py
  # Or with explicit paths:
  uv run python scripts/te_electricity/merge_mc_into_te_gaps.py \\
    --config data/te_electricity/electricity_queries.json \\
    --te-dir data/te_commodities/te_news_by_symbol \\
    --mc-dir data/te_electricity/mc_news \\
    --raw-dir data/te_electricity/raw \\
    --output-dir data/te_electricity/merged_news
"""
...
def load_stems(config_path: Path) -> list[str]:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Merge MC news into TE news for electricity stems (gap dates only)"
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("data/te_electricity/electricity_queries.json"),
        help="Electricity config (stem list)",
    )
    parser.add_argument(
        "--te-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Directory of MC per-stem JSONs",
    )
    parser.add_argument(
        "--raw-dir",
        type=Path,
        default=Path("data/te_electricity/raw"),
        help="Directory of raw CSVs (defines numeric date range); run prepare_raw.py first",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("data/te_electricity/merged_news"),
    ...
    print(
        f"\nMerged output in {args.output_dir}. Next: run_daily_summaries_by_symbol "
        f"--news-dir {args.output_dir} --output-dir data/te_electricity/te_daily_summaries"
    )
    print("Then: merge_all_te_text --summaries-dir data/te_electricity/te_daily_summaries --data-dir data/te_electricity")
