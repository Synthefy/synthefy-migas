#!/usr/bin/env python3
"""
Fetch Media Cloud news for TE electricity price series (DEU, ESP, FRA, UK, ITA).

Reads data/te_electricity/electricity_queries.json (stem -> name, query), fetches MC
stories for --d1/--d2, optionally filters by title relevance, and writes per-stem JSON
in the format expected by create_daily_summaries (date, llm_summary, url).

Outputs to data/te_electricity/mc_news/ by default. Then run merge_mc_into_te_gaps.py
to fill TE news gaps, then run_daily_summaries_by_symbol and merge_all_te_text.

Media Cloud rate limit: certain endpoints are limited to 2 requests per minute.
Defaults use --delay 35 and --symbol-delay 35. Query syntax: AND/OR capitalized.
...
# Keywords that suggest electricity/energy relevance in title (case-insensitive)
TITLE_RELEVANCE_KEYWORDS = (
    "electricity", "power", "energy", "prices", "price", "wholesale", "spot",
    "Germany", "Spain", "France", "UK", "Britain", "Italy", "European",
    "grid", "renewable", "solar", "wind", "nuclear", "gas", "coal",
    "EEX", "EPEX", "day-ahead", "auction",
)
...
def load_config(config_path: Path) -> dict:
    """Load electricity_queries.json: stem -> {name, query}."""
    with open(config_path, encoding="utf-8") as f:
        return json.load(f)


def date_range_by_years(
...
def fetch_stories_for_date_range(
    ...
    """Paginate story_list and return all stories. Waits delay_seconds before each request."""
    if language:
...
def title_looks_relevant(title: str, stem: str, name: str) -> bool:
    """True if title contains at least one relevance keyword or the series name."""
    if not title:
        return False
    lower = title.lower()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fetch Media Cloud news for TE electricity price series (5 countries)"
    )
    parser.add_argument(
        "--config",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=str,
        default="2018-10-01",
        help="Start date YYYY-MM-DD (default: 2018-10-01)",
    )
    parser.add_argument(
        "--d2",
        type=str,
        default="2026-02-13",
        help="End date YYYY-MM-DD (default: 2026-02-13)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=float,
        default=35.0,
        help="Seconds before each API request (MC rate limit; default 35)",
    )
    parser.add_argument(
        "--symbol-delay",
        type=float,
        default=35.0,
        help="Seconds before first request of each symbol (default 35)",
    )
    parser.add_argument(
        "--no-relevance-filter",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=0,
        metavar="N",
        help="Fetch in N-year date chunks (default 0 = whole range). Use 1 to reduce pages.",
    )
    args = parser.parse_args()

    if mediacloud is None:
        print(
            "Error: mediacloud not installed. Run: uv sync --extra mediacloud",
            file=sys.stderr,
        )
        sys.exit(1)
        ...
        for attempt in range(args.retries + 1):
            try:
                ...
                if attempt < args.retries:
                    ...
                    print(
                        f"    API returned non-JSON. Retry in {backoff}s "
                    ...
                    print(
                        f"    Error: {e}. Try --delay 45 --symbol-delay 45.",
                        file=sys.stderr,
                    )
                ...
                if attempt < args.retries:
                    backoff = 30 * (attempt + 1)
                    print(
                        f"    Retry in {backoff}s (attempt {attempt + 1}/{args.retries + 1})...",
                        file=sys.stderr,
                    )
                    time.sleep(backoff)
                else:
