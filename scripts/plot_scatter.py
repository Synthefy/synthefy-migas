...
def find_best_window_for_dataset(ttfm_mae: np.ndarray, ts_mae: np.ndarray, window_length: int) -> Tuple[int, int]:
    """
...
def load_predictions_from_outputs(results_dir: Path, per_dataset_csv: Path, model_names: list) -> Tuple[dict, np.ndarray]:
    """
...
def plot_sample_level_scatter(results_dir: Path, per_dataset_csv: Path, compare_model: str = "gpt_forecast", window_length: int = None):
    ...
    if use_new_format:
        ...
        predictions, gt = load_predictions_from_outputs(results_dir, per_dataset_csv, models_to_load)
        ...
        if window_length is not None and ts_pred is None:
            print("Warning: timeseries predictions not found, cannot apply window filtering!")
            window_length = None
    else:
        ...
        if window_length is not None:
            ...
            if not ts_pred_path.exists():
                print("Warning: timeseries_pred.npy not found, cannot apply window filtering!")
                window_length = None
            else:
    ...
    if len(compare_pred) != n_samples_expected:
        print(f"Note: {compare_model} has {len(compare_pred)} samples, using first {n_samples_expected} samples.\n")
    ...
    if window_length is not None:
        plots_dir = results_dir / f"sample_scatter_plots_ttfm_vs_{compare_model}_window{window_length}"
    else:
    ...
    model_display_name = model_display_names.get(compare_model, compare_model.replace('_', ' ').title())
    ...
    def get_dataset_display_name(dataset_name: str) -> str:
        ...
        return dataset_display_names.get(dataset_name, dataset_name.replace("_", " ").strip())
    
    # Build mapping from original indices to filtered indices
    original_indices = np.arange(len(compare_mae) + n_filtered)
    filtered_original_indices = original_indices[valid_mask] if n_filtered > 0 else original_indices
    ...
    for idx, row in df.iterrows():
        ...
        if n_filtered > 0:
            dataset_mask = (filtered_original_indices >= start_idx_original) & (filtered_original_indices < end_idx_original)
            dataset_filtered_indices = np.where(dataset_mask)[0]
        else:
            dataset_filtered_indices = np.arange(start_idx_original, min(end_idx_original, len(compare_mae)))
        ...
        if window_length is not None and ts_pred is not None:
            ...
            window_start, window_end = find_best_window_for_dataset(ttfm_mae_dataset, ts_mae_dataset, window_length)
        ...
        fig, ax = plt.subplots(figsize=(3.5, 3.5), dpi=300)  # Single column width for papers
        ...
        # Scatter plot
        ax.scatter(compare_mae_dataset, ttfm_mae_dataset, s=25, alpha=0.6, 
                  c=color_points, edgecolors='black', linewidth=0.3, rasterized=True)
        ...
        ax.plot([min_val, max_val], [min_val, max_val], '--', 
               color=color_diagonal, alpha=0.7, linewidth=1.5)
        
        # Add fit line if available
        if poly_fn is not None and not np.isnan(coeffs[0]):
            x_fit = np.linspace(np.min(compare_mae_dataset), np.max(compare_mae_dataset), 100)
            y_fit = poly_fn(x_fit)
        ...
        print(f"  [{idx+1}/{len(df)}] {dataset_name}: r={correlation:.4f}, R²={r_squared:.4f}, fit: y={coeffs[0]:.3f}x+{coeffs[1]:.3f}, n={n_samples_in_window}{window_info}")
    ...
    if window_length is not None:
        csv_path = results_dir / f"sample_scatter_correlations_ttfm_vs_{compare_model}_window{window_length}.csv"
    else:
        csv_path = results_dir / f"sample_scatter_correlations_ttfm_vs_{compare_model}.csv"
    ...
    if window_length is not None and ts_pred is not None:
        ...
        for idx, row in df.iterrows():
            ...
            all_compare_mae_filtered.extend(compare_mae_dataset[window_start:window_end])
    ...
    dataset_to_original_idx = {row['dataset_name']: orig_idx for orig_idx, row in df_original.iterrows()}
    ...
    for idx, row in df_filtered.iterrows():
        ...
        orig_count_col = "n_eval_samples" if "n_eval_samples" in df_original.columns else "n_samples"
        # Calculate start_idx and end_idx based on the original dataframe
        start_idx = sum([int(df_original.iloc[i][orig_count_col]) for i in range(original_idx)])
        ...
        # Apply window filtering if requested
        if window_length is not None and ts_pred is not None:
            ts_mae_dataset = compute_mae(ts_pred[start_idx:end_idx], gt[start_idx:end_idx])
            window_start, window_end = find_best_window_for_dataset(ttfm_mae_dataset, ts_mae_dataset, window_length)
            compare_mae_dataset = compare_mae_dataset[window_start:window_end]
            ttfm_mae_dataset = ttfm_mae_dataset[window_start:window_end]
        ...
        # Scatter plot
        ax.scatter(compare_mae_dataset, ttfm_mae_dataset, s=15, alpha=0.6, 
                  c=color_points, edgecolors='black', linewidth=0.2, rasterized=True)
        
        min_val = min(np.min(compare_mae_dataset), np.min(ttfm_mae_dataset))
        max_val = max(np.max(compare_mae_dataset), np.max(ttfm_mae_dataset))
        ax.plot([min_val, max_val], [min_val, max_val], '--', 
               color=color_diagonal, alpha=0.7, linewidth=1.2)
        ...
        if n_samples_in_window > 1 and not np.isnan(correlation) and not np.isnan(slope):
            x_fit = np.linspace(np.min(compare_mae_dataset), np.max(compare_mae_dataset), 50)
    ...
    # Add title
    plt.suptitle("Sample-Level MAEs Comparison",# f'TTFM vs {model_display_name}: Sample-Level MAE{title_suffix}', 
                fontsize=12, fontweight='normal')
    
    if window_length is not None:
        summary_path_pdf = results_dir / f"sample_scatter_summary_ttfm_vs_{compare_model}_window{window_length}.pdf"
        summary_path_png = results_dir / f"sample_scatter_summary_ttfm_vs_{compare_model}_window{window_length}.png"
    else:
        summary_path_pdf = results_dir / f"sample_scatter_summary_ttfm_vs_{compare_model}.pdf"
        summary_path_png = results_dir / f"sample_scatter_summary_ttfm_vs_{compare_model}.png"
    ...
    # Scatter plot
    ax.scatter(compare_mae_for_stats, ttfm_mae_for_stats, s=15, alpha=0.4, 
              c=color_points, edgecolors='black', linewidth=0.2, rasterized=True)
    ...
    # Add linear fit
    x_fit = np.linspace(np.min(compare_mae_for_stats), np.max(compare_mae_for_stats), 100)
    ...
    if window_length is not None:
        overall_path_pdf = results_dir / f"sample_scatter_overall_ttfm_vs_{compare_model}_window{window_length}.pdf"
        overall_path_png = results_dir / f"sample_scatter_overall_ttfm_vs_{compare_model}_window{window_length}.png"
    else:
        overall_path_pdf = results_dir / f"sample_scatter_overall_ttfm_vs_{compare_model}.pdf"
        overall_path_png = results_dir / f"sample_scatter_overall_ttfm_vs_{compare_model}.png"
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Create scatter plots comparing TTFM vs other models")
    ...
    parser.add_argument("--compare_model", type=str, default="gpt_forecast",
    ...
    if not per_dataset_csv.exists():
        ...
        if candidates:
            ...
            print(f"Error: {per_dataset_csv} not found and no stats_Context_*_allsamples.csv in {results_dir}")
            return
    
    print(f"Creating scatter plots from: {results_dir}\n")
    
    plot_sample_level_scatter(results_dir, per_dataset_csv, args.compare_model, args.window_length)
