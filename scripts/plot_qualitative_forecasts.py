...
def load_sample_dates_and_text(
    ...
    if sample_start < 0 or sample_end > total_len:
        raise ValueError(f"Sample index {local_idx} out of bounds for dataset with {total_len} rows "
...
def generate_context_summaries(
    ...
    try:
        ...
        try:
            ...
            print("  Skipping context summaries (ttfmeval.model.util.ContextSummarizer not available; install ttfm-eval or add ContextSummarizer for LLM summaries).")
            return samples
    ...
    for i, sample in enumerate(samples):
        ...
        if csv_path.exists():
            try:
                ...
                print(f"  Warning: Could not load dates/text for {sample.dataset_name} sample {sample.local_idx}: {e}")
                sample.dates = None
    ...
    for i, sample in enumerate(samples):
        ...
        if has_text:
            ...
            sample.context_summary = "No text annotations available for this time window."
    
    if skipped_no_annotations > 0:
        print(f"  Skipped {skipped_no_annotations} samples (could not load annotations)")
...
def load_predictions_from_outputs(results_dir: Path, per_dataset_csv: Path) -> Tuple[Dict[str, np.ndarray], np.ndarray, np.ndarray]:
    """
    ...
    model_names = [
        'ttfm', 'timeseries', 'chronos_univar', 'chronos_multivar', 'chronos_emb',
        'chronos_gpt_cov', 'chronos_gpt_dir_cov', 'gpt_forecast', 'timesfm_univar', 
        'tabpfn_ts', 'prophet', 'naive', 'ttfm_timesfm'
    ]
...
def compute_raw_mean_std(
    ...
    df = pd.read_csv(per_dataset_csv)
    sample_count_col = "n_eval_samples" if "n_eval_samples" in df.columns else "n_samples"
    history_means = []
    history_stds = []
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            if n_samples_loaded != n_samples_expected:
                print(f"  Warning: Sample count mismatch for {dataset_name}: "
                      f"expected {n_samples_expected}, got {n_samples_loaded}")
                # Pad or truncate to expected count
                if n_samples_loaded < n_samples_expected:
                    dataset_means.extend([dataset_means[-1]] * (n_samples_expected - n_samples_loaded))
                    dataset_stds.extend([dataset_stds[-1]] * (n_samples_expected - n_samples_loaded))
                else:
...
def find_absolute_best_samples(
    ...
    if per_dataset:
        ...
        for _, row in df.iterrows():
            ...
            print(f"  {dataset_name}: Selecting {len(sorted_local_indices)} best samples "
            ...
            for local_idx in sorted_local_indices:
                ...
                sample = SampleData(
                    ...
                    mae_baseline=mae_model[global_idx],  # Same as ttfm for absolute best
...
def find_best_samples(
    ...
    if per_dataset:
        ...
        for _, row in df.iterrows():
            ...
            print(f"  {dataset_name}: Found {len(better_local_indices)} samples, "
...
def _create_forecast_plot(
    ...
    if use_dates:
        ...
        ax.axvline(x=dates_context[-1], color='gray', linestyle='--', linewidth=1, alpha=0.7)
        
        # Plot input context
        ax.plot(dates_context, input_data, color=COLORS['input'], linewidth=3, label='Historical', zorder=3)
        
        # Plot ground truth
        ax.plot(dates_pred_extended, gt_extended, color=COLORS['ground_truth'], linewidth=3, label='Ground Truth', zorder=4)
        ...
        for model_name in models_to_plot:
            ...
            ax.plot(dates_pred_extended, pred_extended, color=color, linewidth=3, label=display_name,
                    zorder=5 if model_name == 'ttfm' else 4, alpha=0.9)
        ...
        # Shaded forecast region
        ax.axvspan(context_len - 0.5, context_len + pred_len - 0.5, alpha=0.15, color='gray')
        ax.axvline(x=context_len - 0.5, color='gray', linestyle='--', linewidth=1, alpha=0.7)
        
        # Plot input context
        ax.plot(t_input, input_data, color=COLORS['input'], linewidth=3, label='Historical', zorder=3)
        
        # Plot ground truth
        ax.plot(t_pred_extended, gt_extended, color=COLORS['ground_truth'], linewidth=3, label='Ground Truth', zorder=4)
        ...
        for model_name in models_to_plot:
            ...
            color = model_colors.get(model_name, '#95A5A6')
            display_name = MODEL_DISPLAY_NAMES.get(model_name, model_name)
            ax.plot(t_pred_extended, pred_extended, color=color, linewidth=3, label=display_name,
                    zorder=5 if model_name == 'ttfm' else 4, alpha=0.9)
    ...
    # Title
    if show_title:
        display_dataset = sample.dataset_name.replace('_', ' ').replace('with text', '').strip()
    ...
    # MAE annotation
    if show_mae:
        mae_ttfm_str = f"{sample.mae_ttfm:.4f}" if show_normalized else f"{sample.mae_ttfm * sample.history_std:.4f}"
        mae_base_str = f"{sample.mae_baseline:.4f}" if show_normalized else f"{sample.mae_baseline * sample.history_std:.4f}"
        mae_text = f"MAE: TTFM={mae_ttfm_str}, Chronos2={mae_base_str}"
        ax.text(0.02, 0.98, mae_text, transform=ax.transAxes, fontsize=8,
    ...
    if show_summary and sample.context_summary:
        ...
        fig.text(0.5, -0.02, wrapped_summary, ha='center', va='top', fontsize=8,
...
def plot_single_forecast(
    ...
    if has_summary:
        ...
        plain_figsize = (figsize[0] * 1.3, figsize[1] * 0.9)  # Wider and shorter for rectangular look
        ...
        output_path_txt = output_path.parent / f"{output_path.stem}_txt{output_path.suffix}"
        ...
        fig_txt.savefig(output_path_txt.with_suffix('.png'), dpi=300, bbox_inches='tight')
        plt.close(fig_txt)
    else:
...
def plot_multi_sample_comparison(
    ...
    for idx, sample in enumerate(samples):
        ...
        ax.axvline(x=context_len - 0.5, color='gray', linestyle='--', 
                   linewidth=0.8, alpha=0.7)
        
        # Plot input
        ax.plot(t_input, input_data, color=COLORS['input'], 
                linewidth=3,
        ...
        gt_extended = np.concatenate([[last_input], gt_data])
        ax.plot(t_pred_extended, gt_extended, color=COLORS['ground_truth'], 
                linewidth=3, 
        ...
        for model_name in models_to_plot:
            ...
            ax.plot(t_pred_extended, pred_extended, color=color, linewidth=3, 
                    # marker=marker, markersize=4,
                    label=display_name)
        
        # Title - clean up dataset name and add sample number
        display_dataset = sample.dataset_name.replace('_', ' ').replace('with text', '').strip()
        ax.set_title(f"{display_dataset} (Sample {sample.sample_idx})", fontsize=10, fontweight='bold')
    ...
    fig.legend(handles, labels, loc='lower center', ncol=len(models_to_plot) + 2,
...
"""
    ...
    parser.add_argument("--models_to_plot", type=str, default="ttfm,chronos_univar",
    ...
    parser.add_argument("--global_selection", action="store_true",
    ...
    parser.add_argument("--grid_samples", type=int, default=10,
                       help="Number of samples per grid plot")
    parser.add_argument("--show_worst", action="store_true",
                       help="Show samples where better_model is WORST (for analysis)")
    parser.add_argument("--absolute_best", action="store_true",
                       help="Show samples with lowest absolute MAE for better_model (no comparison)")
    parser.add_argument("--use_dates", action="store_true",
                       help="Use actual dates from CSV on x-axis")
    parser.add_argument("--generate_summaries", action="store_true",
                       help="Generate context summaries using LLM and add below plots")
    parser.add_argument("--llm_base_url", type=str, default="http://localhost:8004/v1",
    ...
    if args.context_len is None:
        ...
        if match:
            ...
            raise ValueError("Could not auto-detect context_len. Please provide --context_len")
    ...
    if not per_dataset_csv.exists():
        ...
        if candidates:
            ...
            print(f"Error: per_dataset_metrics.csv not found and no stats_Context_*_allsamples.csv in {results_dir}")
            return
    ...
    if use_new_format:
        ...
        predictions, gt, input_data = load_predictions_from_outputs(results_dir, per_dataset_csv)
    else:
        ...
        for sample in best_samples:
            ...
            if csv_path.exists():
                try:
                    ...
                    print(f"  Warning: Could not load dates/text for {sample.dataset_name} sample {sample.local_idx}: {e}")
        print(f"  Loaded dates/text for {sum(1 for s in best_samples if s.dates is not None)} samples")
    ...
    if not args.grid_only:
        ...
        print(f"  Generated {len(best_samples)} individual plots in {len(dataset_dirs_created)} dataset directories")
    else:
