# %% [markdown]
# # Migas-1.5: Text Changes the Forecast
# 
...
bullish_predictive = dedent("""\
    PREDICTIVE SIGNALS: Crude oil has broken out of the recent consolidation \
    and is now climbing in a sustained upward leg, with fresh highs repeatedly \
    overtaking prior peaks as market liquidity tightens. At the same time, \
    inventory draws across key storage hubs are accelerating, reinforcing the \
    view that a durable supply deficit is forming and pushing market \
    participants to add aggressively to long positions.
""").strip()

bearish_predictive = dedent("""\
    PREDICTIVE SIGNALS: Crude oil has entered a sustained downturn, slipping \
    out of its recent range as downside momentum builds and each rebound \
    stalls more quickly than the last. Demand concerns are intensifying \
    alongside visible inventory builds and softer global growth expectations, \
    leading traders to lean harder into short positioning and reinforcing the \
...
plot_scenario_comparison(
    context_vals, fc_original, fc_bullish,
    direction="up",
    title="Bullish counterfactual vs. original",
    timestamps=all_timestamps, ax=ax1,
)
plot_scenario_comparison(
    context_vals, fc_original, fc_bearish,
    direction="down",
    title="Bearish counterfactual vs. original",
    timestamps=all_timestamps, ax=ax2,
)
...
# | **Breakout** | How far the forecast extends beyond the historical range |
# | **Trend Score** | Weighted composite of all the above |
    ...
    rows.append(
        {
            ...
            "Breakout": f"{breakout_ratio(fc, context_vals, direction):.4f}",
            "Trend Score": f"{composite_trend_score(fc, direction, context_vals):+.3f}",
        }
    )
