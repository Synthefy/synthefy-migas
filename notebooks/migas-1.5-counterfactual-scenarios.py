...
_ctx_tensor = (
    torch.tensor(context_unscaled).unsqueeze(0).unsqueeze(-1)
)  # (1, seq_len, 1)
_chronos2_out = evaluate_chronos(
    _ctx_tensor, pred_len=pred_len, device="cuda", chronos_device="cuda"
)
...
print(
    f"Forecast trend score: {composite_trend_score(chronos2_forecast, 'up', context_unscaled):+.3f}"
)
...
print(
    f"Trend score (chronos2): {composite_trend_score(chronos2_forecast, 'up', context_unscaled):+.3f}"
)
print(
...
ax.plot(
    t_pred,
    np.concatenate([[last_val], chronos2_forecast]),
    color=COLORS["Chronos-2"],
    lw=2.2,
    ls="--",
    label="Chronos-2 baseline",
    alpha=0.85,
    solid_capstyle="round",
)
ax.plot(
...
ax.set_title(
    "Migas-1.5 scenario fan vs. Chronos-2: the same numbers, three different stories",
    fontsize=11,
    fontweight=600,
)
